...
class ExportSVG(bpy.types.Operator):
    ...
    bl_label = 'Export SVG'
    bl_description = 'Generate an SVG file from active view'
    ...
    def execute(self, context):
        ...
        def objeto_2_bm(context, obj, bm, convertir=False):
            if convertir:
                tmp = obj.to_mesh(context.scene, True, 'PREVIEW')
        ...
        for frame in frame_list:
            ...
            if wm.use_continuar:
                for linea in datos[:-3]: dibujo(linea)
                dibujo('\n\n<!-- new blender session -->\n\n\n')
            else:
            ...
            if wm.algo_color == 'patron':
                ran = str(R.choice(list(range(999))))
            ...
            if wm.join_objs and len(sel) > 1:
                ...
                for i, o in enumerate(sel):
                    tmp = o.to_mesh(sce, True, 'PREVIEW')
                    tmp.transform(o.matrix_world)
                    if not i:
                        join = bpy.data.objects.new('join',tmp)
                        sce.objects.link(join)
                    else:
                        add = bpy.data.objects.new('add',tmp)
                        sce.objects.link(add)
                        add.select = True
                        join.select = True
                        sce.objects.active = join
                        bpy.ops.object.join()
                        bpy.data.meshes.remove(tmp)
                sel = [join]
                    ...
                    # aplicar color por caras
                        ...
                        if wm.algo_color == 'objetos' or wm.algo_color == 'pal_objetos':
                            ...
                            if o.material_slots and o.material_slots[sl].name:
                                col = o.material_slots[sl].material.diffuse_color
                ...
                if wm.extra_bordes != 'nada':
                    ...
                    for e in edg:
                        ...
                        if e.verts[0].index not in V or e.verts[1].index not in V:
                            vis = False
                        if wm.use_sharp:
                            vis = e.use_edge_sharp
                        if vis:
                            vis = e.calc_face_angle() > wm.stroke_ang
                        if vis:
                            test1 = str_xy(ver[e.verts[1].index].co)
                            test2 = str_xy(ver[e.verts[0].index].co)
                            if test1[4] and test2[4]:
                                v1 = test1[3]
                                v2 = test2[3]
                                delta = v1 - v2
            ...
            if wm.join_objs:
                for o in sel_valid: o.select = True
                sce.objects.active = sel_valid[-1]
                tmp = join.data
                sce.objects.unlink(join)
...
class PanelSVG(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        split.operator('export.svg', text='SVG Sequence').render_range = True
        ...
        if wm.algo_edge != 'nada':
            ...
            split = column.split(.8, align=True)
        ...
        split = column.split(0.35, align=True)
...
bpy.types.WindowManager.ruta=bpy.props.StringProperty(name='', subtype='FILE_PATH',
        default=os.path.join(os.path.dirname(bpy.data.filepath), "algo.svg"), description='Save the SVG file')
...
bpy.types.WindowManager.algo_shade=bpy.props.EnumProperty(name='Shading', items=[('nada','0. Nothing', 
        ...
        'Sombreado'), ('indices','3. Indices','Based on face indices'),('profundidad',
        '4. Depth','Distance from camera -local space ramp-'),('suave','5. Soft Shading',
        'Soft Shading'),('posterize','6. Posterization','Reduce the number of shade steps'),
...
bpy.types.WindowManager.obj_rel=bpy.props.BoolProperty(name='Show Hierarchy',
...
bpy.types.WindowManager.use_sharp=bpy.props.BoolProperty(name='Sharp Edges',
        default=False, description='Only export edges tagged as Sharp')
